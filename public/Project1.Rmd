...
data %>% group_by(state) %>% summarize_at(c("cvap", "rep_pres", "dem_pres", "other_pres", "total_votes"), sum) %>%
  ...
  kable("latex", booktabs = T, col.names = c("State", "Voting Age Population", "Votes for Trump", "Votes for Clinton", "Votes for 3rd Party", "Total Votes", "% Turnout", "Winner"), caption = "2016 State Voting Stats") %>% 
  kable_styling(full_width = T, font_size = 9) %>% column_spec(1, width = "3.5cm")
#state mean
data %>% group_by(state) %>% summarise_at(c("cvap", "pct_29", "pct_65", "pct_rural"), mean, na.rm = T) %>%
  kable("latex", booktabs = T, col.names = c("State", "Mean Voting Age Population (VAP)", "Mean % of VAP under 29", "Mean % of VAP over 65", "Mean % of VAP in Rural Areas "), caption = "2016 County Population Means by State") %>% 
  kable_styling(full_width = T, font_size = 9) %>% column_spec(1, width = "3.5cm")
#state median
data %>% group_by(state) %>% summarise_at(c("cvap", "pct_29", "pct_65", "pct_rural"), median, na.rm = T) %>%
  kable("latex", booktabs = T, col.names = c("State", "Median Voting Age Population (VAP)", "Median % of VAP under 29", "Median % of VAP over 65", "Median % of VAP in Rural Areas "), caption = "2016 County Population Medians by State") %>% 
  kable_styling(full_width = T, font_size = 9) %>% column_spec(1, width = "3.5cm")
#state sd
data %>% group_by(state) %>% summarise_at(c("cvap", "pct_29", "pct_65", "pct_rural"), sd, na.rm = T) %>%
  kable("latex", booktabs = T, col.names = c("State", "SD Voting Age Population (VAP)", "SD % of VAP under 29", "SD % of VAP over 65", "SD % of VAP in Rural Areas "), caption = "2016 County Population Standard Deviations by State") %>% 
  kable_styling(full_width = T, font_size = 9) %>% column_spec(1, width = "3.5cm")
#state var
data %>% group_by(state) %>% summarise_at(c("cvap", "pct_29", "pct_65", "pct_rural"), var , na.rm = T) %>%
  kable("latex", booktabs = T, col.names = c("State", "Var of Voting Age Population (VAP)", "Var of % of VAP under 29", "Var of % of VAP over 65", "Var of % of VAP in Rural Areas "), caption = "2016 County Population Variances by State") %>% 
  kable_styling(full_width = T, font_size = 9) %>% column_spec(1, width = "3.5cm")
```
\newpage
...
data %>% group_by(quality_q) %>% arrange(quality_q) %>%
  ...
  kable("latex", booktabs = T, col.names = c("QoL Life Quartile", "Republican Voting Counties", "Democrat Voting Counties", "Percentage of Quartile Voting Republican"), 
...
data %>% group_by(ses_q) %>% arrange(ses_q) %>%
  ...
  mutate(winner_pres = ifelse(rep_pres > dem_pres, "Republican", "Democrat")) %>%
  summarise(rep_counties = sum(winner_pres == "Republican", na.rm = T), dem_counties = sum(winner_pres == "Democrat", na.rm = T), pct_rep = round((rep_counties/(rep_counties + dem_counties))*100)) %>%
  kable("latex", booktabs = T, col.names = c("SES of Life Quartile", "Republican Voting Counties", "Democrat Voting Counties", "Percentage of Quartile Voting Republican"), 
...
data %>% group_by(quality_q) %>% arrange(quality_q) %>%
  ...
  kable("latex", booktabs = T, col.names = c("QoL Quartile", "Republican Voters", "Democrat Voters", "Percentage of Quartile Voting Republican"), caption = "2016 County Quality of Life Quartiles and Voter Populations") %>% 
  kable_styling(full_width = T, font_size = 10, latex_options = "HOLD_position")

data %>% group_by(ses_q) %>% arrange(ses_q) %>%
  filter(ses_q != "NR" & ses_q != "5") %>%
  summarise(rep_voters = sum(rep_pres, na.rm = T), dem_voters = sum(dem_pres, na.rm = T), pct_rep = round((rep_voters/(rep_voters + dem_voters)) * 100)) %>%
  kable("latex", booktabs = T, col.names = c("SES Quartile", "Republican Voters", "Democrat Voters", "Percentage of Quartile Voting Republican"), caption = "2016 County SES Quartiles and Voter Populations") %>% 
  kable_styling(full_width = T, font_size = 10, latex_options = "HOLD_position")
```
...
corr %>% kable("latex", booktabs = T, caption = "2016 National Correlation Matrix", row.names = T) %>%
  kable_styling(full_width = T, font_size = 10, latex_options = "hold_position") %>%
...
corr2 %>% kable("latex", booktabs = T, caption = "2016 Texas Correlation Matrix", row.names = T) %>%
  kable_styling(full_width = T, font_size = 10, latex_options = "HOLD_position") %>%
  column_spec(1, width = "3.5cm")
```
...
```{r}
pam_data%>% mutate(Cluster = as.factor(pam_clust$clustering)) %>% slice(pam_clust$id.med) %>% kable("latex", booktabs = T, col.names = c("QoL quartile", "SES quartile"," % 29 and under", "% 65 and older", "% rural", "% republican", "Cluster"), caption = "2016 national county PAM clustering medians") %>% 
  kable_styling(full_width = T, font_size = 10, latex_options = "HOLD_position")
```
